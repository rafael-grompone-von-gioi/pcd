...
static void usage(void)
{
  ...
  fprintf(stderr,"usage: parallax_cloud_detector W D <img1A> <img1B> ");
  fprintf(stderr,"[...imgNA imgNB] <cloud_mask>\n");
  ...
  fprintf(stderr,"examples: ./parallax_cloud_detector 5 20 imageA.tif");
  fprintf(stderr," imageB.tif out.png\n");
  fprintf(stderr,"          ./parallax_cloud_detector 20 20 R.tif G.tif ");
  fprintf(stderr,"G.tif B.tif out.png\n");
...
int main(int argc, char ** argv)
{
  ...
  /* read images */
  N = (argc - 4) / 2;  /* number of input image pars from number of arguments */
  for(n=0; n<2*N; n++) /* loop on the number of images to read */
    {
      /* read new image */
      int XX,YY,CC;
      double * new_img = iio_read_image_double_split(argv[n+3], &XX, &YY, &CC);
  ...
  parallax_cloud_detector(cloud_mask, images, X, Y, N, W, D);
